...
export class EditingPhase {
    constructor(gameState, uiManager, audioEngine, timer, canvasRenderer, inputController) {
    ...
    start(onComplete) {
        ...
        this.gameState.timers.phaseCountdown = this.gameState.config.phaseCountdownTime;
    ...
    setupUI() {
        ...
        this.uiManager.updateTransportControls('editing', false, 0, this.gameState.config.segmentLength);
    ...
    setupEventHandlers() {
        ...
        this.inputController.registerHandler('editingMouseDown', 'editing', (mouseX, mouseY, canvasIndex) => {
            return this.handleMouseDown(mouseX, mouseY, canvasIndex);
        });

        this.inputController.registerHandler('editingMouseMove', 'editing', (draggedNote, dragStartY, dragStartPitch, mouseY) => {
            this.handleMouseMove(draggedNote, dragStartY, dragStartPitch, mouseY);
        });

        this.inputController.registerHandler('editingMouseUp', 'editing', (draggedNote) => {
    ...
    handleMouseDown(mouseX, mouseY, canvasIndex) {
        ...
        if (clickedEvent) {
            ...
            const selectedSound = this.gameState.selectedSounds[clickedEvent.soundIndex];
            if (selectedSound) {
                this.audioEngine.startEditPreview(selectedSound.audioBuffer, clickedEvent.pitchSemitones);
            }
    ...
    handleMouseMove(draggedNote, dragStartY, dragStartPitch, mouseY) {
        ...
        const newPitch = this.canvasRenderer.constrainPitch(dragStartPitch + pitchChange, -12, 12);
        ...
        if (newPitch !== draggedNote.pitchSemitones) {
            ...
            const selectedSound = this.gameState.selectedSounds[draggedNote.soundIndex];
            if (selectedSound) {
    ...
    startAnimation() {
        const animate = () => {
            if (this.gameState.getState() === 'editing' && this.gameState.playback.isPlaying) {
    ...
    play() {
        ...
        this.uiManager.updateTransportControls('editing', true, this.gameState.playback.currentTime, this.gameState.config.segmentLength);
    }
    ...
    pause() {
        ...
        this.uiManager.updateTransportControls('editing', false, this.gameState.playback.currentTime, this.gameState.config.segmentLength);
    }
    ...
    restart() {
        ...
        this.uiManager.updateTransportControls('editing', this.gameState.playback.isPlaying, 0, this.gameState.config.segmentLength);
    }
    ...
    seekTo(time) {
        ...
        this.uiManager.updateTransportControls('editing', this.gameState.playback.isPlaying, time, this.gameState.config.segmentLength);
    }
